...
Both helpers are called from three sites in `handleEvent` (EventCardMoved, EventRenownAddedToLocation, EventRenownRemovedFromLocation). Putting the condition tracking + notify inside the helper means every call site stays in sync without duplicating logic.
