...
WHY pre-pick list for branching: after `createRenownAddedToLocationEvent` the in-memory `CityLocation->Renown` may still be 0 until the event hub runs; counting “other empty” from the snapshot at click time matches “apply as much as possible” without double-entering state 2 incorrectly.
