...
Will create a new GameState class and add supporting entries to other files. The purpose of which is to enable a card to be chosen from the hand.
...
7. Update the EventHandlers file
...
## Steps to update the EventHandlers file
1. Update the file `modules\js\EventHandlers.js`
2. Create a new entry in the file using the name provided in the parameters
3. Copy the exact pattern used for the entry `highDramaPhase01158`.
4. Place the new entry in the file in alphabetical order in the area where similar named entries are located. 