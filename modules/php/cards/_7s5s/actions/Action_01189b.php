...
class Action_01189b extends EventCityAction
{
    ...
    public function actFromActionWithIds(Game $game, int $state, string $stateName, array $ids): void  
    {
        ...
        if ($state == States::HIGH_DRAMA_PLAYER_TURN_01189b)
        {
            ...
            $movingEvent = EventFactory::createRenownMovingBetweenLocationsEvent($performer->ControllerId, $poo->Location, $location->Name, 1, "{$poo->getInjectCode()}: Moving Renown to adjacent location");
            $movingEvent->batchId = $batchId;
            $game->theah->eventCheck($movingEvent);

            $fromEvent = EventFactory::createRenownRemovedFromLocationEvent($performer->ControllerId, $poo->Location, 1, "{$poo->getInjectCode()}: Moving Renown to adjacent location");
            $fromEvent->batchId = $batchId;
            $game->theah->eventCheck($fromEvent);
    
            $toEvent = EventFactory::createRenownAddedToLocationEvent($performer->ControllerId, $location->Name, 1, "{$poo->getInjectCode()}: Moving Renown to adjacent location", $isMove = true);
