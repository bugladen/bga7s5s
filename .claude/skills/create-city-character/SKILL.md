...
| [pattern-g.md](pattern-g.md) | Pressure-count Influence bonus (`getInfluencePressureValue`) |
...
- `$this->Engaged` — `true` = committed / has acted; `false` = en garde / ready. Italic *En Garde* on ability text is usually a precondition (`!$this->Engaged`), not flavor.
...
| **`<b>En Garde City Action:</b>`** / **`<b>En Garde Action:</b>`** | Still Pattern C (`CharacterAction`). **En Garde is a precondition** (`!$owner->Engaged`) — **not** an Engage cost. Only queue `createCardEngagedEvent` when the printed text has **Engage** before the bullet. Do not confuse with italic *En Garde* pressure passives (Pattern G). See Pattern C "En Garde City Action / En Garde Action". Exemplar: Tijani `_04cd29`. |
...
| **`<b>Gambling Technique:</b>`** | Custom technique under `cards/<expansion>/techniques/`. Gate `IN_DUEL` + `DUEL_GAMBLED` + actor == owner. Not a Trait. See [pattern-e.md](pattern-e.md) "Gambling Technique". Exemplar: Tijani `_04cd29`. |
...
5. CityCharacter: set Negotiable, WealthCost, CityCardNumber; Traits in `TraitNames.php` (add new ones alphabetically). Dual generic Techniques need distinct `setId`s. **En Garde** Action ≠ Engage cost; do not invent "opposing" when the print omits it.
