...
| `modules/php/cards/bas/_04cd15.php` + `actions/Action_04cd15.php` + `States/bas/State_highDramaPhase04cd15*.php` | **Sorcerer grant + City Action deck look.** Passive Pattern B (Sorcerer). Engage-this-card on trigger (no picker before look). Look top 3 of own faction deck → sink any (Pass = none) → reorder rest → may discard to draw (log both accept and decline). Immediate bottom-insert for mid-action sinks (reorder race). Empty look skips to discard-optional via `"04cd15_3"` EVENTS transition. |
