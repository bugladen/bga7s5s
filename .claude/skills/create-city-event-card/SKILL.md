...
The `faf` branch established the rich interactive pattern — separate State classes, dedicated JS files per expansion, multi-player sequential loops via queued transitions. Follow that for City Actions / Reactions. **Pure Forced** (no player choice) stays on the card class only (see `bas/_04cd07`) — do not invent Action/State/JS scaffolding. **Forced that requires choosing** (e.g. "each player must choose one of their characters here") still needs State + JS; it is Forced timing/obligation, not a City Action — see `bas/_04cd11` and [pattern-a.md](pattern-a.md).
...
- Interactive City Actions **and interactive Forced** need expansion-scoped JS (`OnEnteringState.<exp>.js`, etc.). Pure Forced (no choice) does not.
...
3. Put abilities in the correct subdirectory files; wire states + JS when needed - see companions. Dusk begin pickers register under `DUSK_PHASE_BEGIN_EVENTS`, not High Drama EVENTS.
4. Satisfy pre-commit literals; run `php -l` on touched PHP.
5. CityEventCard: set CityCardNumber; pure Forced often lives on the card class itself.
