...
- **`Traits`** must exist in `TraitNames::$TraitsJson` (`modules/php/TraitNames.php`). Add missing ones in alphabetical order. (Memory feedback.) Common traits: `Weapon`, `Melee`, `Ranged`, `Attire`, `Hat`, `Mantel`, `Tabbard`, `Talisman`, `Boon`, `Stealth`, `Unique`, `Alquimia`, `Codex`, plus the faction-themed traits (`Dar Matushki`, `Oathsworn`, `Red Hand`, ...). `_04036` required adding `Codex`.
...
| **"May only equip to your character with N[Stat] or more"** / **"2[Finesse] or more"** | Pattern A — **stat-threshold equip**. Dual-gate on `ModifiedFinesse >= N` (or Combat/Influence sibling). Printed `[Stat]` means current modified value, not the printed base. No trait check. See Pattern A / `_04053`. |
...
| **`<b>Gambling Technique:</b>`** / **`<b>Gambling Maneuver:</b>`** | Pattern E + **Gambling gate** — NOT a trait gate. Always gate **both** `Game::IN_DUEL` and `Game::DUEL_GAMBLED` (actor gambled this round). See Pattern E and `_03043` / `Technique_03002` / `Maneuver_03008` / `_03064` / `_04016`. |
| **"Engage this card • +N [Thrust]"** / **"Engage this card • +N [Parry]"** / **"Engage this card • +N [Riposte]"** (no Gambling keyword) | Pattern E — gate `! $attachment->Engaged` + actor identity; engage on Resolve; `EventDuelCalculateTechniqueValues` `thrust += N` / `parry += N` / `riposte += N`. Mirror `Technique_04017` (Thrust) / `Technique_04026` (Parry) / `Technique_04054b` (Riposte) / `Technique_03018`. Do **not** add `DUEL_GAMBLED` just because the card also has a passive gamble-reveal clause. |
