...
description: Implement or finish a Faction Attachment (modules/php/cards/<expansion>/_NNNNN.php where the class extends FactionAttachment). Use this skill whenever the user asks you to implement, finish, scaffold, or wire up a Faction Attachment, or when they reference a faction-deck attachment whose class extends FactionAttachment and has unimplemented Text. Triggers on phrases like "implement this faction attachment", "finish _NNNNN" (when it extends FactionAttachment), "wire up the equip restriction", "equip to an opposing character", "add the City Reaction to this weapon", "the equipped Strega does X", "opponents cannot move Home", "equipped character cannot move", "destroy this attachment at the end of High Drama", "sink this card", "use a condition for the restriction", "during a duel while their adversary is wounded", "reveals an additional card when they gamble", "gains +1 Finesse during a duel", "after a Hunter or Berserker equips this card", "heal a wound", "Gambling Technique", "Gambling Technique +1 Parry", "at the end of your round each participant gains a threat", "Engage this card • +1 Thrust", "Engage this card • +1 Parry", "Engage this card • +1 Riposte", "If your participant is an Academic or Hunter", "the adversary discards a card", "pressure at this location would succeed", "pressure fails instead", "difference of 1 or less", "Ignore that wound", "opponent's ability would wound", "2[Finesse] or more", "2[Combat] or more", "Leather Spaulders", "Sabre", "Sturdy Shield", "Offhand", "Shield", "Crossbow", "Pompon", "Diplomat pet", "Ciphered Tome", "May only equip to your Academic", "Move a Renown from this location", "another location with more Renown", "two City Actions", "Alquimia", "Codex", "Armor", or natural-language descriptions of a faction-deck attachment (Weapon / Attire / Hat / Mantel / Tabbard / Talisman / Trinket / Compass / Chains / Unguent / Crossbow / Animal / Pet / Dog / Codex / Armor / Shield that lives in a player's faction deck and equips onto one of their characters).
---

# Creating a Faction Attachment

Faction attachments are *player-deck cards* that equip onto one of the controller's own characters from hand (paying `WealthCost`), modifying that character's stats and granting Forced abilities, Actions, Reactions, Techniques, or Maneuvers. They live in the player's faction deck and discard pile — unlike `CityAttachment`s, they are *not* drawn from the city deck. Most belong to a named faction; **Neutral** faction attachments also exist (`initializeFaction('Neutral')` — e.g. Harpoon `_03064`, Lodestone `_03065`, Shackles `_03066`, Main Gauche `_02056`, Leather Spaulders `_04053`, Sabre `_04054`, Sturdy Shield `_04055`) and still use `FactionAttachment` + `CardNumber` (not city-deck numbering). A few (`CanEquipToOpponents`) equip onto **opposing** characters instead — see Pattern A.
...
- **`Traits`** must exist in `TraitNames::$TraitsJson` (`modules/php/TraitNames.php`). Add missing ones in alphabetical order. (Memory feedback.) Common traits: `Weapon`, `Melee`, `Ranged`, `Attire`, `Hat`, `Mantel`, `Tabbard`, `Talisman`, `Boon`, `Stealth`, `Unique`, `Alquimia`, `Codex`, `Armor`, `Shield`, plus the faction-themed traits (`Dar Matushki`, `Oathsworn`, `Red Hand`, ...). `_04036` required adding `Codex`; `_04055` required adding `Shield`.
...
- **`OffHand = true`** when printed text says **Offhand** — does not count against the one-Armor / one-Weapon limit; still limited to one Offhand per character (enforced in `UtilitiesTrait::characterHasAttachmentOfType` / `Reaction_AttachmentTypeLimit`). See `_03044`, `_01047`, `_02056`, `_04055` (Armor/Shield Offhand).
...
| **"May only equip to your character with N[Stat] or more"** / **"2[Finesse] or more"** / **"2[Combat] or more"** | Pattern A — **stat-threshold equip**. Dual-gate on `ModifiedFinesse >= N` / `ModifiedCombat >= N` / `ModifiedInfluence >= N`. Printed `[Stat]` means current modified value, not the printed base. No trait check. See Pattern A / `_04053` (Finesse) / `_04055` (Combat). |
| **"After a \<Trait\> equips this card"** / **"After a Hunter or Berserker equips this card • …"** | Pattern D — **Reaction trigger trait gate**, NOT Pattern A. Anyone may equip; the Reaction only offers when the host has the listed trait(s). Do **not** implement `canAttachTo` / `eventCheck(Equipping)` unless text also says "May only equip…". See Pattern D / `_04016`. |
...
| **"When the equipped character gambles, reveal an additional card"** (no "while …" condition) | Pattern B''' gamble half only — always-on `getNumberOfGambleCardsToReveal` (Gallegos `_01101` / `_04017`). **Not** a Gambling Technique. A separate `<b>Technique:</b>` on the same card is still a normal Technique unless the keyword literally says Gambling. |
| **"During a duel, while …"** / **"gains +N [Stat]"** (conditional) / **"reveals an additional card when they gamble"** (conditional) | Pattern B''' — **duel-scoped conditional**. Do **not** use constructor `*Modifier`. Stat: Benci-style applied flag + `createCharacter*ModifiedEvent`; recompute on DuelStarted / Wounded / Healed / Swapped / Equipped; clear on DuelEnd / Unequipped. Gamble +1: override `getNumberOfGambleCardsToReveal` with the **same** condition (`_04006`). No Action/Reaction class unless a keyword is also printed. See Pattern B. |
...
| **`<b>Forced:</b> At the end of High Drama • Destroy this attachment`** | `EventHighDramaPhaseEnd` + `isAttached()` → notify → unequip → `createAttachmentDiscardedFromPlayEvent(..., asEffect=true)`. Trigger mirror `_01025_Burden`; destroy chain mirror `_01153` / `_01050`. Unequip clears any while-equipped condition. |
| **`<b>Action:</b>` or `<b>City Action:</b>`** | Pattern C — `AttachmentAction` (no `AttachmentCityAction` base). City → `cardInCity` gate. Immediate resolve (no picker): `_03065`. Engage-this-card / choose-location move: `_03055`. **"Sink this card"** cost: unequip → removeFromPlay → faction-deck bottom (`Technique_02055` / `Action_03065`). Two printed Actions → `Action_NNNNNa` / `Action_NNNNNb` + separate GameStates when filters/effects differ (`_04036`). |
| **"Move a Renown from this location to another City location"** | Pattern C + **renown relocate** — source is the host's location (not a source picker). Dest = other City slots. `Moving` + `Removed` + `Added(..., isMove=true)` with one `batchId`. Gate current `Renown > 0`. Do **not** copy `Action_01007`'s "pick the from-location" UI. See `_04036` / `Action_04036a`. |
| **"Move … to another location with more Renown"** | Pattern C choose-location. Dest filter is **strictly greater** `$location->Renown` than the host's current slot. Home has **no Renown track** (treated as 0) so it never qualifies — omit Home even if the text is not City-only; JS stays city-only (`04cd01`), not Home-capable `03055`. See `Action_04036b`. |
...
| **"When a pressure … would succeed by one or fewer"** / **"succeeds with a difference of 1 or less • … fails instead"** / **"at this location"** (pressure fail) | Pattern D + **pressure-fail** — `EventLocationPressured` (`success` + `difference <= 1`); `"at this location"` → `$event->location == owningCharacter->Location`; engage cost → apply fail in `performReaction` (not Risk pay / `ICancelReaction`). `deletePressureResultEvents` + `createLocationPressureResultEvent(..., success=false)`. `HIGH_PRIORITY` offer. Mirror Objection `Reaction_01027` (Risk) → attachment `Reaction_04026`. Do **not** copy Objection's wealth-pay / `EventRiskReactionTriggered` path. |
| **"When an opponent's ability would wound the equipped character"** / **"Ignore that wound"** / **"(The wound is not taken.)"** | Pattern D + **ignore-wound** — cancel-first on `EventCharacterBeingWounded`; clone + re-queue on Pass (`skipNextEvent`); Ignore + engage clears the clone (wound never applied). Opponent-ability gate: non-empty `abilityId` + `getAbilityById` + ability owner `ControllerId !=` attachment controller. Target = equipped character only. Engage cost (not wealth pay). Mirror Cascade `Reaction_02059` (Risk) → attachment `Reaction_04053`. Do **not** copy Cascade's hand / `EventRiskReactionTriggered` pay path. No `HIGH_PRIORITY` needed (wound canceled immediately). See Pattern D. |
...
| **`<b>Gambling Technique:</b>`** / **`<b>Gambling Maneuver:</b>`** | Pattern E + **Gambling gate** — NOT a trait gate. Always gate **both** `Game::IN_DUEL` and `Game::DUEL_GAMBLED` (actor gambled this round). See Pattern E and `_03043` / `Technique_03002` / `Maneuver_03008` / `_03064` / `_04016` / `_04055`. |
| **`<b>Gambling Technique:</b> +N [Parry]`** / **"+N [Thrust]"** / **"+N [Riposte]"** (no engage) | Pattern E + Gambling gate + Calculate `+= N`. **Do not** `new Technique_PlusOneParry()` / `PlusOneThrust` / `PlusOneRiposte` + `setId` — those only gate `IN_DUEL`, so the ability would offer every duel round without gambling. Own class (`Technique_04055b`). `setId` on PlusOne* is only for plain non-Gambling free Techniques (Sabre `_04054` Thrust / Pavel `_01120`). |
| **"Engage this card • +N [Thrust]"** / **"Engage this card • +N [Parry]"** / **"Engage this card • +N [Riposte]"** (no Gambling keyword) | Pattern E — gate `! $attachment->Engaged` + actor identity; engage on Resolve; `EventDuelCalculateTechniqueValues` `thrust += N` / `parry += N` / `riposte += N`. Mirror `Technique_04017` (Thrust) / `Technique_04026` / `Technique_04055a` (Parry) / `Technique_04054b` (Riposte) / `Technique_03018`. Do **not** add `DUEL_GAMBLED` just because the card also has a Gambling Technique or a passive gamble-reveal clause — keep gates per printed keyword. |
| **"If your participant is a \<Trait\> or \<Trait\>, …"** (inside Technique/Action text) | **Resolve-time effect gate**, NOT availability and NOT Pattern A. Pay costs / apply unconditional halves first; only the "If …" clause is trait-gated. See Pattern E / `_04017` (Academic **or** Hunter → adversary discards). |
| **"the adversary discards a card"** (Technique) | Pattern E + **adversary hand picker** — Maya `Technique_01093` / Íñigo `Technique_03039` / `_04017`. `createTechniqueTransitionEvent($adversary->ControllerId, $attachment->Id, …)` when hand non-empty; empty hand → notify + skip (checklist 9). Attachment-hosted `sourceId` = attachment. |
| **"At the end of your round, each participant gains a threat"** | Pattern E + **deferred EndOfRound** — `IsActive` on `EventResolveTechnique`; on `EventDuelEndOfRound` when `actorId == owningCharacter->Id`, queue `createThreatModifiedEvent(1, 1)`. Clear `IsActive` on fire / `EventTechniqueCanceled` / `EventDuelEnd`. No GameState. See Pattern E / `_04016` / `Reaction_02039` (threat factory sibling). |
...
A single attachment commonly combines several — `_01075` mixes an equip restriction, a passive trait grant, AND a City Action. `_03007` mixes an equip restriction with a Sorcerer City Reaction. `_03043` mixes a passive trait grant with a Gambling Technique. `_03064` is a single Gambling Technique that stamps a remainder-of-duel condition (no equip restriction). `_03065` mixes a while-equipped condition restriction (B'') with an immediate-resolve sink + move-Home City Action (C). `_03066` mixes opponent-equip + performer-Finesse gate (A), while-equipped cannot-move (B''), and Forced end-of-HD destroy. `_04006` mixes multi-trait equip (A) with duel-scoped wounded-adversary +1 Finesse **and** +1 gamble reveal (B''') — no keyword ability class. `_04016` (Drachenblut) mixes a self-equip heal Reaction (Hunter/Berserker trigger gate, not equip restriction) with a Gambling Technique that defers +1/+1 threat to EndOfRound. `_04017` (Jägerarmbrust) mixes always-on gamble +1 reveal (B''', Gallegos) with a **normal** Technique (engage + +1 Thrust + Resolve-time Academic/Hunter adversary discard) — passive gamble text does **not** make the Technique a Gambling Technique. `_04026` (Pompon) mixes Diplomat equip (A) with a pressure-fail AttachmentReaction (Objection `_01027` adapted to engage cost + location gate) and a normal engage +1 Parry Technique — no GameState. `_04036` (Ciphered Tome) mixes Academic equip (A) with two City Actions (`Action_04036a` renown relocate + `Action_04036b` move-to-more-Renown). Both cost "Engage this card" (using one disables the other that turn). Separate HD GameStates — dest filters and resolve differ. No cross-talk. `_04053` (Leather Spaulders) mixes Finesse-threshold equip (A) with an ignore-wound AttachmentReaction (Cascade `_02059` adapted to engage cost) — no GameState. `_04054` (Sabre) mixes free `Technique_PlusOneThrust` (`setId`) with engage +1 Riposte (`Technique_04054b`). `_04055` (Sturdy Shield) mixes Offhand + Combat-threshold equip (A) with engage +1 Parry (`Technique_04055a`) and Gambling +1 Parry (`Technique_04055b`) — separate classes; do not put `DUEL_GAMBLED` on the engage half or reuse `Technique_PlusOneParry` for the Gambling half.
