...
| **"Engage this card • +N [Thrust]"** (no Gambling keyword) | Pattern E — gate `! $attachment->Engaged` + actor identity; engage on Resolve; `EventDuelCalculateTechniqueValues` `thrust += N`. Mirror `Technique_04017` / `Technique_03018`. Do **not** add `DUEL_GAMBLED` just because the card also has a passive gamble-reveal clause. |
