...
| **"Engage this card • +N [Thrust]"** / **"Engage this card • +N [Parry]"** (no Gambling keyword) | Pattern E — gate `! $attachment->Engaged` + actor identity; engage on Resolve; `EventDuelCalculateTechniqueValues` `thrust += N` or `parry += N`. Mirror `Technique_04017` (Thrust) / `Technique_04026` (Parry) / `Technique_03018`. Do **not** add `DUEL_GAMBLED` just because the card also has a passive gamble-reveal clause. |
