...
- **"a location" vs "City location" destinations:** literal "City" → city slots only. Unqualified "a location" → include `LOCATION_PLAYER_HOME` when the filter can match. JS must use `makeHomeEndcapMarkerSelectable` for Home — do not copy city-only enter handlers from `03032`/`03045`. **Exception:** Home has no Renown track. "More Renown than current" can never match Home — omit it and keep JS city-only (`04cd01`), even if the text says "a location" (`Action_04036b`).
- **Two printed Actions:** `Action_NNNNNa` / `Action_NNNNNb` + separate HD GameStates when dest filters or resolve differ (`_04036`). Shared picker hides which effect is resolving. Both "Engage this card" → `$attachment->Engaged` is the only shared gate.
- **Renown relocate ("from this location"):** dest picker only; source is `$owner->Location`. Do **not** copy Aldo `Action_01007` (that picks the from-location). Batch `Moving` + `Removed` + `Added(..., isMove=true)` with one `getNextEventBatchId()`. Gate current `Renown > 0`.
- **New GameState-class HD states** (`State_highDramaPhaseNNNNN` under `States/<expansion>/`): register the constant in `States.php` and `"NNNNN"` on `HIGH_DRAMA_PLAYER_TURN_EVENTS` in `states.inc.php`. Do **not** add a parallel block in `states.7s5s.php` (old 7s5s array). a/b ids: `4040361` / `4040362` (mirror `4030381` / `4030382`).
...
- **Passive gamble reveal ≠ Gambling Technique.** "When the equipped character gambles, reveal an additional card" is B''' on the attachment. A co-printed `<b>Technique:</b>` stays normal unless the keyword says Gambling (`_04017`).
- **"If your participant is a Trait…"** inside Technique text = Resolve-time effect gate only — not `isAvailableToPlayer`, not Pattern A (`Technique_04017`).
- **Adversary discards a card:** Maya hand picker (`01093` / `03039` / `04017`) — `createTechniqueTransitionEvent` + attachment `sourceId` + empty-hand notify. Not reveal-then-discard (`_03043`), not cancel-unless-discard (`_03044`).
