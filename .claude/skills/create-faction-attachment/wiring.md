...
- **While-equipped conditions** (Lodestone `_03065`): same JS/condition plumbing, but stamp on `EventAttachmentEquipped` / clear on `EventAttachmentUnequipped` (not DuelEnd). Enforce in `Character::eventCheck`. Opponent Home-move detection uses `sourceId` ControllerId — **never** `initiatingPlayerId` (`Maneuver_01033` sets initiating to the victim). See Pattern B''.
