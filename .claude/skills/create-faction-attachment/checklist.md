...
16c. **Two printed Actions:** `Action_NNNNNa` / `Action_NNNNNb`; separate GameStates when dest filters or resolve differ. Do **not** add `states.7s5s.php` blocks for new GameState-class HD states. Both "Engage this card" → no extra cross-talk beyond `$attachment->Engaged`. See `_04036`.
16d. **Renown relocate ("from this location"):** dest picker; source is host location — do **not** copy `Action_01007`'s from-location picker. `Moving` + `Removed` + `Added(..., isMove=true)` with one `batchId`. Gate current `Renown > 0`. Literal City dest → no Home.
16e. **"More Renown" dest filter:** strictly greater `$location->Renown`. Home has no Renown track (treated as 0) so omit Home even if text says "a location"; JS city-only (`04cd01`). See `Action_04036b`.
