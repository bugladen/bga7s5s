...
- `modules/php/cards/faf/_03052.php` (Yevgeni, All-Seeing Cyclops) — `Character` with (1) an **unlabelled beginning-of-Dusk optional effect implemented as a Continuous Reaction** that privately looks at the top three City Deck cards, sinks exactly one to the bottom, and privately reorders the rest; and (2) a **Gambling Technique that privately looks at the adversary's whole hand**. Canonical distinctions: sink ≠ discard; look ≠ reveal; state args must be private.
...
62. **For "Gambling Technique: Look at adversary's hand":** standard gambling + actor gates and nonempty-hand gate; transition to owner (not adversary); `argsForStatePrivate`; read-only chooseList + Done/Pass; do not use public Reveal/multiplayer acknowledge. Pattern reference: `Technique_03052`, contrast `Technique_03043`. See Pattern E "Privately look at adversary's hand".