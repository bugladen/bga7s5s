...
| **"<Name> cannot issue [Combat] challenges"** (stat-specific ban) | **`eventCheck` only** — do NOT override `canChallenge()`. Gate `EventChallengeIssued` on `challengerId == $this->Id` AND `CHALLENGE_STAT == STAT_COMBAT`. Reference: `_03028` Térence. |
| **"<Name> may only issue [Combat] challenges to \<traits\>"** (Combat target restriction) | **`eventCheck` only** — when `CHALLENGE_STAT == STAT_COMBAT`, validate defender traits; return early for other stats. Finesse/Influence unrestricted. No `canChallenge()` override. Reference: `_02013` Wilhelm. |
...
| **"After an opponent's effect engages your other <Trait> at this location, they may en garde"** (unlabelled After…may) | Pattern D **Continuous** on `EventCardEngaged`. Gates: `!$event->canceled`, Owner in play, engaged card is **other** controlled Trait at Owner's location, **opponent's effect** (not Challenge auto-engage). Opponent gate = `Reaction_03031::isOpponentAbility` shape (`source` ControllerId is live opponent, or in-play action owner) — **`sourceId == 0` is framework auto-engage, not an effect**. Stash engaged id; En Garde / Pass → `createCardEngardedEvent`. Re-check `Engaged` in `performReaction` (`runEventHubAfterCards=true` so Engaged may still be false during `handleEvent`). See Pattern D "Opponent engages your other Trait → they may En Garde". Reference: `Reaction_04021` Aimée. |
...
| **"While <Owner> is participating in a duel, he gains <b>Trait</b> …"** (self trait grant for duel duration) | Pattern A on the **card class**: `addTrait` / `removeTrait` on **self** with a `$DuelTraitsApplied` flag. Lifecycle = Térence participation (`EventDuelStarted` when challenger/defender, clear on `EventDuelEnd`, apply/clear on `EventChallengerSwapped` / `EventDefenderSwapped`). Do **not** override `hasTrait`. Contrast Daniella `Action_03013` (tags **opposing** characters) and attachment equip grants (`_03043`). See Pattern A "Gains traits while participating in a duel". Reference: `_04033` Iago. |
...
| **"While <Owner> has a <Trait> card in her dueling line, the adversary cannot perform Maneuvers"** | Pattern A **`eventCheck`** on `EventResolveManeuver` gated on `$event->adversaryId == $this->Id` + Owner is duel participant + ≥1 matching trait in `LOCATION_DUELING_LINE` for Owner's controller. WHY `adversaryId` (Maryam) not `EventManeuverActivated` (Maneuver_01129): adversary-scoped — Owner's own Maneuvers stay legal. See Pattern A "Adversary cannot Maneuver while trait in dueling line". Reference: `_04012` Raven; Technique sibling `Technique_01186` Maryam. |
...
| **"While you control a <Trait> at this location, <Owner> gains +N[StatA] and +M[StatB]"** (multi-stat location aura) | Same Angeline lifecycle + stale-DB compensation as the single-stat row. Prefer **Ise/Benci flag ±1 per stat** (not Angeline absolute `base + bonus`) — attachments also mutate Finesse/Influence/Combat. Apply/remove both stats only on flag transition. Reference: `_04022` Axelle; single-stat absolute sibling `_03026` Angeline. |
...
| **"Opposing <b>Sorcerers</b> have -N[Influence]"** (location-scoped aura on **opponents**, not a self-buff) | Pattern A on the **card class**. Debuff **other** characters (track `$DebuffedIds` / conditions), not Owner. Apply/remove via `createCharacterInfluenceModifiedEvent(±N)` + named `Game::*_CONDITION` + `conditionsApplied`/`Removed` notifs (Contempt `_01143` shape). Scope = **same location as Owner** (`getOpposingCharactersAtLocation` + Sorcerer trait) — **not** global. **Home → empty list** (shared Home string). Stale-DB on `EventCardMoved` like Benci/Axelle. Clear all when Owner leaves play / moves Home. See Pattern A "Opposing trait -N Influence at Owner's location". Reference: `_04032` Giacinto; global sibling `_01143`. |
...
| **"When a character opposing <Owner> equips …, it gains +1 cost"** | Pattern A on the card: override `getEquipDiscount` → `$discount -= 1`. Gates: opposing performer + same location + `cardInCity($performer)`. Negative discount = cost increase. Reference: `_01092` Makepeace. Scheme sibling `_03063`. |
| **"<i>En Garde</i> — When an opposing character recruits a <b>Mercenary</b>, they gain +1 cost"** | Pattern A on the card: override **`getParleyDiscount`** → `$discount -= 1`. **There is no `getRecruitDiscount`.** Recruit cost always flows through `getParleyDiscount` (`FrameworkActionsTrait` / `stRecruitComputeDiscount`). Italic En Garde → `!$this->Engaged`. Opposing gates match Makepeace + `isControlled()` on self. Do **not** use `getEquipDiscount` for recruit taxes. See Pattern A "Cost +1 taxes (equip vs recruit)". Reference: `_04011` Hans. |
...
| **`<b>En Garde City Action:</b> Target opposing … • Move <Owner> and that character to the same adjacent City unless their controller reveals their hand and discards a card.`** | Pattern C multi-step. En Garde = `!$owner->Engaged` (precondition, not Engage cost). Step 1 target (`IAbilityThatTargetsCharacters`). Step 2 = **Reveal Hand or Pass** (Crimson Roger `02036a` shape — not pick-discard yet). Pass → Owner picks adjacent city (`getAdjacentCityLocations`, `engage=false` both moves, shared `batchId` — Depose `04028`). Reveal → log every hand card + stash ids in `Game::REVEALED_CARDS` → **ACTIVE_PLAYER chooseList for Owner** (Giacinto views; Ok) → game bridge sets active = hand owner → hand owner discards one revealed card. **Do not** use multiplayer acknowledge for "show chooseList to Owner" — auto-ack pref / zombies skip it. Enter view/discard steps via `createTransitionEvent` + `nextState("")` through EVENTS. See Pattern C "Reveal hand or Pass → Owner chooseList → discard or move both". Reference: `Action_04032` Giacinto. |
| **"Move target <b>Mercenary</b> at this location Home. (… in play and not available)"** | Pattern C one-step picker. Text says **target** → `IAbilityThatTargetsCharacters`. Eligible = `hasTrait("Mercenary")` + **`isControlled()`** at owner's location ("not available" = not uncontrolled city recruit fodder). No "opposing" printed → own Mercenaries are legal. Move `engage=false` when Engage is not printed. Mirror Makepeace move-Home UX. See Pattern C "Move target Mercenary Home". Reference: `Action_04011`, sibling `Action_01092`. |
...
| **"Destroy [an] attachment"** (effect, any context) | Canonical recipe: `createAttachmentUnequippedEvent` → `eventCheck` → queue; then `createCardDiscardedFromPlayEvent(..., $asEffect = true)`. Do **not** invent `createAttachmentDestroyedEvent` — it does not exist. **CityAttachment** → `createCardAddedToCityDiscardPileEvent` instead (same split as `Character::unEquipAllAttachments`). Skip `FakeAttachment`. Reference: `Action_01174`, `Maneuver_01142`, `Action_03038b`, `Technique_04013`. |
| **"Technique: Destroy an attachment equipped to <Owner> • +N[Thrust]"** | Pattern E picker (Katain `02011` / Croc `02026b` button list) + destroy recipe in `actFromTechniqueWithId` + `$event->thrust += N` on `EventDuelCalculateTechniqueValues` (and `+N` Threat on `EventGenerateChallengeThreat` like `Technique_PlusTwoThrust`). Availability: `IN_DUEL` + actor-is-owner + ≥1 non-Fake attachment. `createTechniqueTransitionEvent("NNNNN")` → `DUEL_CHOOSE_TECHNIQUE_NNNNN`. Character already has `TechniqueTrait` — do **not** re-`use TechniqueTrait` / re-`implements IHasTechniques` on the card class. See Pattern E "Destroy Owner's attachment for +N Thrust". Reference: `Technique_04013` Tomas; self-destroy sibling `Technique_DestroyPlusOneThrust` (no picker). |
...
| **"… Other characters cannot intervene."** (on a challenge Action) | Pattern F with a dedicated `*_CHALLENGE_TYPE`. Unconditional ban: `interventionCheck` throws for the type; **empty** `$charactersCanIntervene` in `ArgumentsTrait` (UX); add type to `Reaction_02058`'s skip list (Jump In must not prompt). No refuse wiring unless printed. Engage follows trichotomy separately. See Pattern F "Other characters cannot intervene". Reference: `Action_04012` Raven, `Action_01123` Valeri, `Action_02034` Torvo. |
...
| **"Reaction: During a duel, after an opposing adversary announces their combat card • Your participant gains a threat. If <Owner> is en garde, the adversary also gains a threat."** | Pattern D on `EventCombatCardAnnounced`. Gates: `IN_DUEL`, `isAvailable`, announcer `playerId != owner.ControllerId`, owner controller has a duel participant. Map threat via `createThreatModifiedEvent(challenger, defender)` — your side = participant whose `ControllerId` matches Owner's; En Garde rider = `!$owner->Engaged` (not an Engage cost). Owner need not be the participant. Risk in-hand sibling `Reaction_02039` adds to both unconditionally. See Pattern D "Adversary announces combat card → threat". Reference: `Reaction_04022` Axelle. |
| **"<b>En Garde Reaction:</b> After <Owner> moves to a City location • Reveal top N of your deck. You may equip a revealed attachment … paying all costs. Discard any then sink the rest."** | Pattern D multi-stage **inside** `playerReaction` (no dedicated states). Trigger = `Reaction_03025` self-move + En Garde `!$Engaged`. Click-to-pay like Tomas — **never** `PAY_STATE_EQUIP_ATTACHMENT` (move can fire outside High Drama). Discard-from-revealed-deck = Action_01134 notify+`moveCard`; sink = `createCardAddedToFactionDeckEvent(..., false)`. WHY no chooseList states: `"NNNNN"` would need wiring on every EVENTS dispatcher Monet can move from. Reference: `Reaction_04023` Monet. |
| **`<b>En Garde City Reaction:</b>`** / **`<b>En Garde Reaction:</b>`** | Still Pattern D. **En Garde is a precondition** (`!$owner->Engaged`) at trigger — **not** an Engage cost. Pair with `cardInCity` when City. Same semantics as En Garde City Action (Tijani `_04cd29`). Reference: `Reaction_04003a` Desideria. |
| **"When a duel occurs at this location, at the beginning of the first round • Remove one threat from your participant"** (En Garde Reaction) | Pattern D on **`EventDuelNewRound`** with `$event->round == 1` (not `EventDuelStarted` — text says "beginning of the first round"). En Garde = `!$owner->Engaged`. Location = `$owner->Location` matches challenger **or** defender location. Map "your participant" via `ControllerId` → `createThreatModifiedEvent(-1, 0)` or `(0, -1)`. Valid-target gate: owner's participant must have threat > 0 (`Game::CHALLENGER_THREAT` / `DEFENDER_THREAT`) before queuing — no useless Pass-only prompt. Remove-only / your-participant-only sibling of Leja `Reaction_01203` (add **or** remove from either participant). See Pattern D "Duel at location, first round → remove your participant's threat". Reference: `Reaction_04031` Andare. |
| **"After your <Trait> at this location is destroyed during a duel or by an opponent's effect • … Put them in your hand"** | Pattern D on `EventCharacterDestroyed`. Cause gate: `IN_DUEL` **OR** opponent-sourced lethal wound marked on `EventCharacterWounded` (`EventCharacterDestroyed` has **no** `sourceId`). Hand return = locker/discard → `RemovedFrom*` + `AddedToHand`. **If mid-duel, defer Hand until `stDuelEnd`** — immediate Hand makes `stDuelNextPlayer` nullify leftover threat. See Pattern D "Destroyed ally → hand (duel / opponent effect)". Reference: `Reaction_04003a`. |
| **"After <Owner> performs a Sorcerer ability • wound her • Draw"** (City Reaction, no Sorcerer keyword on the Reaction) | Pattern D on `EventSorcererAbilityPlayed`; `sourceId` OR `performerId`; **do not** `implements ISorcererAbility`. Reference: `Reaction_04003b`, Cesca `Reaction_03001`. |
...
| **"When a non-Artifact attachment … would be put into a discard pile • Equip it … instead, paying all costs"** | Pattern D **cancel-and-reissue on discard** + character picker + click-to-pay equip. Listen on **both** `EventCardDiscardedFromPlay` and `EventCardAddedToCityDiscardPile` (`runEventHubAfterCards=true`). Stash host on `EventAttachmentUnequipped` (AttachedToId cleared before discard; discard `sourceId` is not always the host). Cost = `WealthCost − getEquipDiscount`; do **not** use `PAY_STATE_EQUIP_ATTACHMENT`. Decline re-queues with `cancelDeclinedByCardIds`. See Pattern D "Would-be-discarded attachment → equip paying costs". Reference: `Reaction_04013` Tomas. |
...
| **"+1[Thrust] or +1[Parry]"** (Technique choice before Calculate) | Pattern E. On `EventResolveTechnique`: `createTechniqueTransitionEvent` (HIGHEST_PRIORITY) into `DUEL_CHOOSE_TECHNIQUE_NNNNN` button state; stash `$UseThrust` in `actFromTechniqueWithId`; apply on `EventDuelCalculateTechniqueValues`. **Prefer `createTechniqueTransitionEvent` over old Vissenta `01013`'s `createTransitionEvent`+HIGH_PRIORITY** — Framework queues Resolve then Calculate; choice must finish before Calculate reads the flag. Button-only JS (`OnUpdateActionButtons` only). See Pattern E "+1 Thrust or +1 Parry choice". Reference: `Technique_04033` Iago; older sibling `Technique_01013`. |
...
| **"At the start of the adversary's next round, you may add a threat to <Owner>"** (deferred optional after Technique) | Pattern E. Arm `$PendingThreatChoice` on Resolve; on `EventDuelNewRound` when `actorId != owner.Id`, clear flag and `createTechniqueTransitionEvent` with the **same** key `"NNNNN"` — `DUEL_NEW_ROUND_EVENTS` routes it to `DUEL_NEW_ROUND_NNNNN` (id `510` + cardId), while `DUEL_CHOOSE_TECHNIQUE_EVENTS` still uses that key for the earlier Thrust/Parry picker (Lorenzo `01090` dispatcher-scoped pattern). Add Threat / Pass buttons; map threat onto Iago via Id → challenger/defender deltas (`createThreatModifiedEvent` — Axelle shape). Skip/clear if discard/locker; clear on cancel / DuelEnd. See Pattern E "Deferred optional effect on adversary's next round". Reference: `Technique_04033`; NewRound sibling `Technique_01090`. |
...
| **"Technique: Copy the effects of a Technique on your other <Trait> at this location. (Not one of their attachments.)"** | Pattern E Dame clone pipeline **without** Engage (unless Engage is printed). Sources = `$character->getTechniques()` on other controlled Traits at Owner's location — **never** attachment techniques. Skip `isAvailableToPlayer` on sources (actor is Owner, not the Trait). Reject `getOwningAttachment() !== null` on pick. JS: technique buttons like `03051` / `02055` (`OnUpdateActionButtons` only). See Pattern E "Copy a Technique on your other Trait (not attachments)". Reference: `Technique_04021a` Aimée; attachment-copy sibling `Technique_03051`. |
| **`<b>En Garde Technique:</b> +1[Thrust]`** (or +N Thrust / Riposte / Parry) | Pattern E. Prefer subclassing the matching generic (`Technique_PlusOneThrust`, etc.) and gate **`!$owner->Engaged`** — En Garde is a **precondition**, not an Engage cost. During `IN_DUEL` also require actor-is-owner. Outside duel leave the generic's challenge-threat path open when En Garde. Pre-commit: `extends Technique_PlusOneThrust` still matches `extends Technique` — keep `// EventTechniqueCanceled handler not needed`. See Pattern E "En Garde Technique +N Thrust". Reference: `Technique_04021b` Aimée; plain +1 sibling `Technique_PlusOneThrust` / Terrell `_01042`. |
| **`<b>Technique:</b> +N[Riposte/Thrust/Parry]. If <Owner> is engaged, gain Lethal.`** | Pattern E. Subclass the matching `Technique_PlusOne*` generic for the stat bonus. **`$owner->Engaged` (ready) is the opposite of En Garde (`!$Engaged`)** — do not gate availability on Engaged; check at `EventDuelCalculateTechniqueValues` and queue `createGainLethalEvent` only when `Engaged`. Duel-only Lethal → Calculate handler only (no `EventGenerateChallengeThreat`). Override `isAvailableToPlayer` for actor-is-owner — parent generic only gates `IN_DUEL`. Pre-commit: keep `EventTechniqueCanceled` comment when extending `Technique_PlusOne*`. See Pattern E "+N stat with Lethal if Owner engaged". Reference: `Technique_04031` Andare. |
| **`<b>Technique:</b> +1[Thrust] or +1[Parry]. At the start of the adversary's next round, you may add a threat to <Owner>.`** | Composite Pattern E: Thrust/Parry choice row **plus** deferred NewRound optional threat row above. One Technique class; two states (`DUEL_CHOOSE_TECHNIQUE_NNNNN` + `DUEL_NEW_ROUND_NNNNN`); shared transition key. Reference: `Technique_04033` Iago. |
