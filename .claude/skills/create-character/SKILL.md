...
| **"Your equipped character moves to this location. Then, destroy their attachment to draw …"** | Two-step Pattern C. "Equipped" = your character with ≥1 non-`FakeAttachment`. Strict "moves to" → exclude characters already at the owner's location (and thus the owner herself when she is there). No "target" in text → **no** `IAbilityThatTargetsCharacters`. Move with `engage=false` when Engage is not printed. Attachment step: button list (Adelheide `01194`), not board highlight. Destroy = unequip + `EventFactory::createAttachmentDiscardedFromPlayEvent` (city vs faction); capture `WealthCost` **before** destroy; then queue `WealthCost + 1` draws. Parenthetical "must be destroyed to draw" → no Pass on the attachment step. Reference: `Action_03038b`. |
| **"Move <Owner> to your Leader's location. Then, en garde your attachment there."** | Pattern C. Availability: `cardInCity`, Leader exists, Owner ≠ Leader location — **no attachment gate** (no cost before `•`; effects L→R if applicable). Queue `createCardMovingEvent(..., $engage=false)`; if ≥1 Engaged non-Fake attachment will be at destination, `createTransitionEvent` into attachment **button** picker and `createCardEngardedEvent`; else `createActionResolvedEvent` (skip). See Pattern C "Move to Leader then En Garde attachment". Reference: `Action_03051`, `Action_02007` / `Action_02040`. |
...
| **"Destroy [an] attachment"** (effect, any context) | Canonical recipe: `createAttachmentUnequippedEvent` → `eventCheck` → queue; then `createAttachmentDiscardedFromPlayEvent(..., $asEffect = true)` (routes city vs faction discard). Do **not** invent `createAttachmentDestroyedEvent` — it does not exist. Reference: `Action_01174`, `Maneuver_01142`, `Action_03038b`. |
...
| **`<b>Technique:</b>` / `<b>Maneuver:</b>`** | The Character lineage already brings `TechniqueTrait`. Add `IHasManeuvers` + `ManeuverTrait` for maneuvers. Implement under `cards/<expansion>/techniques/` or `cards/<expansion>/maneuvers/`. See Pattern E. **"Adversary" before the • (cost/condition)** → gate `IN_DUEL` in availability; **"adversary" only in the effect after the •** → do not add `IN_DUEL` for that word alone. |
| **`<b>Gambling Technique:</b>`** | Same as Technique, but `isAvailableToPlayer` **must** gate **both** `Game::IN_DUEL` and `Game::DUEL_GAMBLED` (actor gambled for their combat card this round). Do not rely on `DUEL_GAMBLED` alone. See Pattern E "Gambling Technique gate". |
