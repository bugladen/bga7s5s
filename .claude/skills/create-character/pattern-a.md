...
- For wound *movement* abilities (the heal+wound recipe, `Action_02010` / Cesca `Action_03001`): the wound half is queued from the action's owner with the action's id as `abilityId`. Same filter blocks the wound half. **But heal is queued first** — if the player is still allowed to pick Kaspar, the Strega loses the wound and it vanishes. So eventCheck alone is not enough for move-wound UX/correctness.
- **Also override `canBeWoundedByOpponentAbilities(): bool` to return `false`** on the card class (default on `Character` is `true`). Opponent wound / move-wound abilities that let the player pick a destination must filter on this in their target list and `isValidTargetForAbility` (Cesca `Action_03001` / `Reaction_03001`). Mirror `canIntervene` / `canChallenge`.
