...
3. **Legacy Reaction shape (`_01133` only unless Eddie asks):** pay-state `RiskReaction` Engage/Pass on `EventEnteringPayState` — same WillEngage semantics. Prefer the GameState for new work. After Engage, **must** call `calculateInHandPayDiscount` (same footgun as Song — early EventHub calc sees `WillEngage=false`). Both Engage and Pass set `ABNORMAL_FLOW`.
...
7. **Heal pool:** "another character at this location" with no "you control" → any other character at the performer's location with `Wounds > 0` (mirror `Action_01091`'s unrestricted heal pool; exclude the performer for "another"). **Wounds are not a play cost** — do **not** gate `getPerformersForAction` / Maneuver `isAvailable` on a wounded heal target (or wounded participant). Empty heal pool after pay → still fire Sorcerer start/played + ActionResolved and skip the chooser (no soft-lock). Chooser / resolve may still filter `Wounds > 0` as an effect filter.
