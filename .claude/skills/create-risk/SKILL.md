...
- `modules/php/cards/faf/_03048.php` (Wily) — **Gambled combat-card cost discount (Pattern E) + Scoundrel Maneuver that moves all in-round threat to the adversary (Pattern C.6).** Discount via `getManeuverFromCombatCardDiscount` when `Game::DUEL_GAMBLED`. Maneuver: Riposte += `getCurrentDuelThreat(actor)` in calc (DB riposte moves threat; contrast Turais `Technique_02012` which uses Parry to *remove*). Exemplar for "if this card was gambled, -1 cost" and for threat-pool maneuvers that stay pure-calc (no sub-state).
- `modules/php/cards/faf/_03056.php` (Astute) — **City Action: target opposing character • if their controller does not control this location, they claim it and you move a Renown (Pattern A.4).** Pronoun trap: **they** = target's controller (claim via targeted character), **you** = active player (Renown move). Gate on claim-control mismatch + `canLocationBeClaimedBy` + Renown ≥ 1. Two states: character chooser → EVENTS (claim) → Renown destination. Exemplar for opponent-claim (not you-claim) + Renown-move payoff on one City Action.
