---
name: create-risk
description: Implement or finish a Risk card (modules/php/cards/<expansion>/_NNNNN.php where the class directly extends Risk). Use this skill whenever the user asks you to implement, finish, scaffold, or wire up a Risk, or when they reference a card whose class extends Risk and has unimplemented Text. Triggers on phrases like "implement this risk", "finish _NNNNN" (when it extends Risk), "wire up the maneuver", "add the city action on this risk", "adversary has more wounds … -1 cost", "two Duelist Maneuvers", "equip this card to target opposing", "text box as blank", "Fate's Silence", "En Garde Action", "Engage a Melee Weapon or Eisenfaust attachment", "No More Words", "Target opponent chooses", "issues a challenge to your performer", "first combat card gains +1 Riposte", "Rattle the Rigging", "Unravel the Thread", "reveals this card while gambling", "reveal additional cards equal to Influence", "sink up to two cards from a discard pile", "Seek Each Devil", "Target an enemy character at an adjacent City location", "Academic or Hunter … -1 cost", "each other player who controls a Sorcerer or Monster … discards", or natural-language descriptions of a Risk card (faction-deck combat card with Riposte/Parry/Thrust, played as a maneuver during duels, sometimes carries a City Action / Action / Reaction).
---
...
| [pattern-b.md](pattern-b.md) | plain Action / B.1 / B.2 RiskAttachment equip / B.3 En Garde Action forced-enemy challenge / B.4 sink discard + draw + sink self / B.5 Target adjacent enemy → move performer → filtered multi-discard / B.6 Engage attachment → Combat challenge |
| [pattern-c.md](pattern-c.md) | Maneuver / C.1–C.9 |
| [pattern-d.md](pattern-d.md) | Reaction / D.1–D.5 (D.5 = deck-reveal CardReaction, not RiskReaction) |
| [pattern-e.md](pattern-e.md) | Passive discounts / Forced (E.1) / Action-only Leader or performer-trait discount (E.2) / text-box blank (E.3) |
...
- **`IRiskThatTargetsCharacters` / `IAbilityThatTargetsCharacters`:** mark when the printed text uses **"Target"** / **"target"** (or similar Rules-Team wording) and the ability hands the player a character chooser — **including friendly choosers**. Compare `_01083`, `_01115`, `_03008`, `_03011`, `_03034`, `_03056`, `_03057`, `_03058`. The Action/Maneuver/Reaction implements `IAbilityThatTargetsCharacters` (`isValidTargetForAbility`); the Risk gets `IRiskThatTargetsCharacters`. **Do not** mark either for: location-chooser Actions (`_03009`, `_03032`, `_03045`); hand-discard choosers; fixed-target Reactions (`_03046`, `_03012`); choosers whose text never says "target" (e.g. `_03060` "Heal … from another character", `_03068` "They must move an en garde character", `_03069` "swap … with your other character", `_03071` "Engage an opposing character" — still a UI chooser, but Cesca `Reaction_01008` must not copy); or **"Target opponent"** player choosers (`_04009` — opponent then picks a character; Cesca is character-target only). Performer selection via `RequiresPerformerSelected` alone is also not a "targets characters" chooser. Contrast `_01078` **"Target enemy character"** (you pick the character → Cesca yes).
...
| **`<b>En Garde Action:</b>`** / **`<b>En Garde City Action:</b>`** (on a Risk) | Still Pattern B (`RiskAction` / `RiskCityAction`). **En Garde is a precondition** (`!$performer->Engaged`) — **not** an Engage cost. Only queue `createCardEngagedEvent` when the printed text has **Engage** before the `•`. Do not confuse with italic *En Garde* pressure passives or the "en garde" verb (`createCardEngardedEvent`). |
| **"Target opponent chooses one of their characters opposing your performer. The chosen character issues a [Combat] challenge to your performer"** (± **"If your performer is a Duelist, their first combat card gains +1[Riposte]"**) | Pattern B.3 — player-target → opponent character chooser → forced enemy issues challenge **to** your fixed performer. Mint custom `CHALLENGE_TYPE` **off** auto-engage. No Cesca ("Target opponent" ≠ character Target). Optional Duelist first-combat-card Riposte sticky on the Action. See `_04009`. Invert of `_01078` (you pick enemy challenger; they pick your defender). |
...
| **"When your performer reveals this card while gambling • Reveal additional …"** (± **"Sorceries gain +N [Parry] this round"**) | Pattern D.5 — **`CardReaction`**, not `RiskReaction`. Card is peeked from the **faction deck** (not hand-paid). Offer **after** chooseList is visible via `createTransitionEvent` (priority 8) under `DUEL_GAMBLE_REVEALED_EVENTS` — not `createReactionTransitionEvent` / early `playerReaction`. Hub must `addCardToWorld` revealed deck cards. Round-lasting Parry → Game global + EventHub combat-card calc (card may leave world). See `_04010`. |
| **"Sink up to N cards from a single discard pile. Then, draw … and sink this card"** (± Sorcerer) | Pattern B.4 — `RiskAction`: always pile chooser (all player discards + City Discard, even empty) → multi-select up to N (Pass = 0; exclude self already in discard) → sink (player→owner faction bottom; City→City Deck) → draw → sink self. See `_04010`. |
| **"Target an enemy character at an adjacent City location • Move your performer there. Then, each other player who controls a Sorcerer or Monster there discards a card"** (± En Garde) | Pattern B.5 — character Target (Cesca yes) + move `engage=false` + trailing **multi-active** filtered discard (not sequential turns; sticky `$PlayersToDiscard` on Action). Not B.1 location chooser. See `_04018`. |
| **`<b>En Garde Action:</b> Engage a <b>Melee Weapon</b> or <b>Eisenfaust</b> attachment equipped to your performer • They issue a [Combat] challenge to target opposing character`** | Pattern B.6 — En Garde precondition + **attachment** Engage cost (not performer) → shared challenge target chooser. Mint custom `CHALLENGE_TYPE` **on** auto-engage list but **not** `NORMAL` (Back on `highDramaChallengeActionChooseTarget` only for `NORMAL`). `"They"` = performer in engine. See `_04019`; attachment filter mirror Yield `_02020`. |
...
| **"This card has -1 cost if your Leader is a [Trait] …"** / **"While your Leader is a [Trait], this card has -1 cost"** / **"While your performer is an Academic or Hunter, this card has -1 cost"** (Action/City Action present; **no** Maneuver printed) | Pattern E.2 — `getActionFromHandDiscount` on the Action (`_01159`, `_01160`, `_03071`, `_04018` performer-trait). Do **not** invent a discount-only Maneuver; combat-card pay stays full WealthCost. |
...
| **"This card has -1 cost if your Leader is a Villain or Pirate"** / **"While your Leader is a Villain/Hero/Diplomat, … -1 cost"** / **"While your performer is an Academic or Hunter, … -1 cost"** (with Action only) | Pattern E.2 — `getActionFromHandDiscount` on the Action; null-check Leader **or** `$performer`; Id-gate the Action. See `_03071`, `_01160`, `_01159`, `_04018`. |
...
3. Ability files in `actions/` / `maneuvers/` / `reactions/`; mark `IRiskThatTargetsCharacters` on the Risk only when printed text says **"Target"/"target"** **and** the ability hands **you** a character chooser (paired with `IAbilityThatTargetsCharacters` on the ability). Skip location-only / fixed-trigger / hand-discard / no-"target"-wording choosers (`_03060`, `_03069` swap, `_03071` engage opposing) and **"Target opponent"** player choosers (`_04009`).
4. Wire states + JS when you add card-specific sub-states — see [wiring.md](wiring.md).
5. Satisfy pre-commit literals (ActionResolved / ManeuverCanceled / hand `==` for **RiskReaction** / setUsed+isAvailable for CardReaction|AttachmentReaction|RiskReaction / Sorcerer start+played). **Do not** put `LOCATION_HAND` on Pattern D.5 `CardReaction` deck-reveal Reactions — that guard is RiskReaction-only.
