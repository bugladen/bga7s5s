...
| **"Add a Renown to any location. If you have the fewest Renown, you may move a Renown to an adjacent location instead"** | Pattern A: default **add** (city location pick). Optional **move-instead** only when the player has **unique** fewest score Renown (**no ties**) **and** ≥1 city location has Renown>0. UX mirrors `_01152`: Confirm Location adds; "Move a Renown Instead" → `actFromCardPass` (needs `PlayerActions.js` `onPass` map) → source pick → adjacent dest. Fewest via `getPlayerReknown`; unique lowest only (Eddie / `_04034` — ties do not unlock move even without printed "(Fewest cannot tie.)"). Reference: `_04034`; always-offer-move sibling `_01152`; fewest-then-add sibling `_01144`. |
...
| **"Add a City Card to [Fixed Location]. Then add a Renown to a different location"** | Pattern A: queue `createCityCardAddedToLocationEvent((int)$top['id'], $fixed)` from `getCardsOnTopOfCityDeck(1)` (cast id — raw deck row). Guard empty deck+discard (notify; still offer Renown). Then one pick state whose `locationIds` **exclude** the fixed location (same UX as `_04014`). MEDIUM_PRIORITY transition after the city-card event. "Different" = ≠ the City Card destination even when no card was added. Reference: `_04025` (No Rest — Bazaar), city-card sibling `_01149` (Docks, no Renown pick). |
...
| **`<b>Strega Reaction:</b>`** / **`<b>Mercenary City Action:</b>`** / **`<b>Merchant …:</b>`** / **`<b>Diplomat …:</b>`** / **`<b>Duelist …:</b>`** / **`<b>Musketeer …:</b>`** / **`<b>Hero …:</b>`** / **`<b>Villain …:</b>`** / **`<b>Scoundrel …:</b>`** / **`<b>Red Hand …:</b>`** | Trait-prefixed keywords are **mechanical performer-trait gates**, NOT Sorcerer abilities. The chosen performer must have that trait (enforce via `hasTrait("Strega")` / `"Duelist"` / `"Merchant"` / `"Hero"` / `"Villain"` / `"Scoundrel"` / `"Red Hand"` etc.). Do NOT `implement ISorcererAbility` for these. When the Reaction has no pickable performer in the trigger text, search for a controlled traited character (at the relevant location if the text says opposing; or anywhere in play for phase-end "each Merchant you control" counts). Reference: `_03006` (Strega), `_04004` (Duelist), `_04005` (Red Hand), `_04025` (Merchant), `_03061` (Hero), `_03062` (Villain), `_03063` (Scoundrel). |
...
| **`<b>… Reaction:</b> At the end of Planning • …`** (Look/Pass, not Forced) | **Not Pattern F.** Reaction listens on `EventPhasePlanningEnd` + scheme at Home + trait gate + preconditions; queue `createReactionTransitionEvent` (Look/Pass). Follow-on chooseList states still use `PLANNING_PHASE_END_<NNNNN>` under **`PLANNING_PHASE_END_EVENTS`** — queue `createTransitionEvent(..., "NNNNN", $this->Id)` with **4th arg = reaction Id** so `actFromCardWithIds` routes to `actFromReactionWithIds`. Same card-number key may also sit on the resolve map. Reference: `_04025` / `Reaction_04025`. |
| **"Look at the top three cards of your deck, and an additional card for each \<Trait\> you control. Draw two of them and sink the rest"** | Planning-End Reaction subtype. Look count = 3 + controlled traited chars (`getCharactersInPlayByPlayerId` + `hasTrait`). Snapshot via `getCardsOnTopOfPlayerFactionDeck` into `Game::CHOSEN_CARD`. **"Look at" is private** (`argsForStatePrivate`) — not Otto-style Reveal-to-all. If looked ≤2, auto-draw all and skip the pick state. If >2, exact-2 multi-select (`onMultipleChooseListCardsConfirmed` + `EventHandlers` enable on `cardsToDraw`). Draw = `createCardRemovedFromPlayerFactionDeckEvent` + `createCardAddedToHandEvent` (Otto). Sink = `createCardAddedToFactionDeckEvent(..., false)` (bottom of faction deck, not discard). Skip offering when 0 drawable. Reference: `_04025`; look/reorder sibling `Reaction_03052` (City Deck); sink-from-look sibling `_02005`. |
